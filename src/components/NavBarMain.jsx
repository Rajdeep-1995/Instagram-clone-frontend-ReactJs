...
import Ilogo from "../media/instagramNameLogo.png";
import { SearchOutlined } from "@ant-design/icons";
import { isAuthenticated } from "../services/auth";
import ProfileHomeDropDown from "./dropdowns/ProfileHomeDropDown";
import { useNavigate } from "react-router-dom";

const NavBarMain = () => {
  const [searchValue, setSearchValue] = useState("");
  const [showSearchList, setShowSearchList] = useState(false);
  const { profilePhoto } = isAuthenticated() && isAuthenticated().user;
    ...
    <>
      <div className="n_main">
        <div className="n_logo">
          <img src={Ilogo} alt="Instagram" width={130} height={45} />
        </div>
        <div className="n_search">
          {!showSearchList && (
            <>
              <lable className="n_search_icon_lable">
                <SearchOutlined />
              </lable>
              <label className="n_search_lable">Search</label>
            </>
          )}
          <input
            className="n_input_field"
            type="search"
          ...
          />
          {showSearchList && (
            <div className="n_search_list">
              <p className="n_r_title">Recent</p>
            </div>
          )}
        </div>
        <div className="n_icons">
          <div onClick={() => navigate("/")} className="svg_icon">
            <svg
              aria-label="Home"
              color="#262626"
              fill="#262626"
          ...
          <div
            ...
            className="n_user_avatar"
          >
            <img src={profilePhoto} alt="username" width={24} height={24} />
            {dropDownOpen && <ProfileHomeDropDown />}
          </div>
        </div>
      </div>
    </>
